  ...
  return new Promise((resolve, reject) => {
    ...
    Promise.all(requests).then(async (promiseResponses) => {
      ...
      if (groupRoleData) {
        ...
        let groupThumbails = {body: data: [] }
        for (let i = 0; groupRoleData.lenth - i *100 > 0 ) {
          const thumbails = await constructRequest(`https://thumbnails.roblox.com/v1/groups/icons?groupIds=${groupRoleData.map(data => data.group.id).slice(i*100, (i+1)*100).join(',')}&size=150x150&format=Png&isCircular=false`)
          groupThumbails.body.data = groupThumbails.body.data.concat(thumbails.body.data)
          i++
        }
