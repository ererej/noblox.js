  ...
  return new Promise((resolve, reject) => {
    ...
    Promise.all(requests).then(async (promiseResponses) => {
      ...
      if (groupRoleData) {
       ...
       let groupThumbnails = {body: {data: [] }}
        for (let i = 0; groupRoleData.length - i *100 > 0 ) {
          const thumbnails = await constructRequest(`https://thumbnails.roblox.com/v1/groups/icons?groupIds=${groupRoleData.map(data => data.group.id).slice(i*100, (i+1)*100).join(',')}&size=150x150&format=Png&isCircular=false`)
          groupThumbnails.body.data = groupThumbnails.body.data.concat(thumbnails.body.data)
          i++
        }
